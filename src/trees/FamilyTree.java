...
public class FamilyTree
{
    ...
    private static class TreeNode
    {
        ...
        ArrayList<TreeNode> collectAncestorsToList()
        {
            ...
            for (TreeNode childNode: children) {
            	if(!ancestors.contains(childNode.parent)) {
            		ancestors.add(childNode.parent);
            	}
            }
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Node with that name not found");
		TreeNode node2 = root.root.getNodeWithName(name2);			// node whose name is name2
		if (node2 == null)
			throw new TreeException("Node with that name not found");
